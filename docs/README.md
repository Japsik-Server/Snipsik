...
- **초경량 유저 해시 슬러그**: 별도 링크 DB 없이 슬러그의 유저 해시(CRC32 -> Base36)를 통해 소유권을 O(1)로 식별 (`{랜덤N자리}-{유저해시}`).
...
- **Twitter/X fixupx 자동 변환**: 감시 채널 내 twitter/x 링크를 Discord 임베드 최적화를 위해 fixupx.com으로 자동 변환.
- **Strict TypeScript, Turso & 보안 강화**: `any` 타입 배제, Turso(LibSQL) + Drizzle ORM, KeyedMutex & OCC 동시성 제어, SSRF 방어 및 회복 탄력적 캐시 복구.
